...
export default function ContributionEventsPage() {
  ...
  // Enforce status rules when date changes
  ...
  // Allowed statuses based on date
  ...
  return (
    <div>
      <SectionHeading
        title="אירועי תרומה"
        description="תכנן וערוך אירועים קבוצתיים כגון ימי צילומים, סדנאות וספרינטים."
        action={
        ...
        <div className="mb-8 bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Date */}
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">תאריך אירוע *</label>
                <input
              ...
              <div>
                ...
                {form.event_date < today && (
                  <p className="text-xs text-amber-600 mt-1">תאריך עבר — הסטטוס חייב להיות "התקיים" או "בוטל".</p>
                )}
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="לדוגמה: יום צילומים — בית הספר"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                />
              </div>
              ...
              {/* Location */}
              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-gray-700 mb-1">מיקום (לא חובה)</label>
                <input
                  value={form.location}
                  onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                  placeholder="לדוגמה: סטודיו 4, תל אביב"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                />
              </div>
              ...
              {/* Contribution type */}
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">סוג תרומה ברירת מחדל</label>
                <select
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  כמות ברירת מחדל{selectedType?.unit ? ` (${selectedType.unit})` : ''}
                </label>
                <input
                  type="number" min={0} step="any"
                  value={form.default_amount}
                  onChange={e => setForm(f => ({ ...f, default_amount: e.target.value }))}
                  placeholder={selectedType?.unit ?? 'כמות'}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                />
              </div>
            </div>
            ...
            {/* Participants */}
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-2">משתתפים מיועדים</label>
            ...
            {form.status === 'occurred' && form.contribution_type_id && form.default_amount && (
              <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-800">
                עם שמירה, תירשם תרומת <strong>{selectedType?.name}</strong> בכמות <strong>{form.default_amount} {selectedType?.unit ?? ''}</strong> לכל {form.participant_ids.length || 'כל'} המשתתפים שנבחרו.
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                {saving ? 'שומר…' : editingId ? 'עדכן אירוע' : 'צור אירוע'}
              </button>
            </div>
          </form>
        </div>
      ...
      {events.length === 0 ? (
        <div className="text-center py-20 text-gray-400">אין אירועים עדיין.</div>
      ) : (
